Exceptions are unscheduled events that disrupt the normal flow of execution (interrupts or faults)
...
2. Turn off all writes for the instruction with the exception, as well as for all instructions that follow that instruction, until the trap is processed. 
